...
#input times you would like to plot
timesc = [0,4,15,50,100]

for i in range(len(timesc)):
    k = int(timesc[i]/Swimmers.dt)
    
    lns1 = axs[i].plot(Swimmers.xm,PAC[k],color='r',label='Plankton',linewidth=2.5)
    ax2 = axs[i].twinx()  #create a second axes that shares the same x-axis
    lns2 = ax2.plot(Swimmers.xm,CAC[k],color='b',linestyle='--',label='Chemical',linewidth=2.5)
    
    axs[i].ticklabel_format(style='sci', scilimits=(2,2),axis='y',useMathText=True)
    ax2.ticklabel_format(style='sci', scilimits=(-1,-1),axis='y')
    
    axs[i].set_xlim(0,Swimmers.right)
    axs[i].set_xlabel(r'$x$',fontsize=30)
    axs[i].locator_params(nbins=5,axis='x')
    axs[i].tick_params(labelsize=18)    
    axs[i].yaxis.get_offset_text().set_fontsize(16)
    axs[i].set_ylim(0,max(PAC[-1])+10)
    ...
    axs[i].set_title(r'$t = {0}$'.format(timesc[i]),fontsize=30)
    ...
    if (i == 0):
        axs[i].set_ylabel(r'Plankton Density', fontsize=22, color='r')

    if (i == 4):
        ax2.set_ylabel(r'Chemical Concentration', color='blue',fontsize=22,rotation=-90,labelpad=30)

plt.savefig('./Figures/Fig4_4a.png')
```
...
ani = matplotlib.animation.FuncAnimation(fig2, animate, init_func=init, frames=range(0,int(len(PAC)/Swimmers.dt),4), 
                                         interval=1, blit=True,repeat=False)

#ani.save('./Figures/Fig4_4a_Supplemental_Video.gif',writer='pillow')
```
...
```{python}
Swimmers = PM.PlankChem(depFcn=DM.atanDep,left=0,right=5,d1=.2,d2=4,N=417,
...
#input times you would like to plot
timesc = [0,4,15,100,149.99]

for i in range(len(timesc)):
    k = int(timesc[i]/Swimmers.dt)
    
    lns1 = axs[i].plot(Swimmers.xm,PAS[k],color='r',label='Plankton',linewidth=2.5)
    ax2 = axs[i].twinx()  #create a second axes that shares the same x-axis
    lns2 = ax2.plot(Swimmers.xm,CAS[k],color='b',linestyle='--',label='Chemical',linewidth=2.5)
    ...
    axs[i].set_title(r'$t = {0}$'.format(round(timesc[i])),fontsize=30)
...
plt.savefig('./Figures/Fig4_4b.png')
```
...
ani = matplotlib.animation.FuncAnimation(fig2, animate, init_func=init, frames=range(0,int(len(PAS)),4), 
                                         interval=1, blit=True,repeat=False)
ani.save('./Figures/Fig4_4b_Supplemental_Video.gif',writer='pillow')
```
...
```{python}
Swimmers = PM.PlankChem(depFcn=DM.linAtanDep,left=0,right=5,d1=.2,d2=4,N=417,
                        dt=0.004,depMaxStr=0.01, depThreshold=0.25, depTransWidth=0.04, delta = 0.015)
...
#input times you would like to plot
timesc = [0,4,15,80,120]

for i in range(len(timesc)):
    k = int(timesc[i]/Swimmers.dt)
    
    lns1 = axs[i].plot(Swimmers.xm,PAL[k],color='r',label='Plankton',linewidth=2.5)
    ax2 = axs[i].twinx()  #create a second axes that shares the same x-axis
    lns2 = ax2.plot(Swimmers.xm,CAL[k],color='b',linestyle='--',label='Chemical',linewidth=2.5)
...
plt.savefig('./Figures/Fig4_4c.png')
```
...
ani = matplotlib.animation.FuncAnimation(fig2, animate, init_func=init, frames=range(0,int(len(PAL)/Swimmers.dt),4),
                                         interval=1, blit=True,repeat=False)
ani.save('./Figures/Fig4_4c_Supplemental_Video.gif',writer='pillow')
```

## Figure 6d

This plot shows the total chemical over time for the three different deposition functions. We must make sure to extend the plot so that all deposition functions have the same time. 

```{python}
fig, ax = plt.subplots(figsize=(7,5),tight_layout=True)

colors = ['#000000','#009E73','#D55E00'] 
times = np.linspace(0,150,len(CTC))
ax.plot(times,CTC,color=colors[0],linestyle = '--',linewidth=3,label=r'$f_1(c)$')
ax.plot(times,CTS,color=colors[1],linestyle = '-',linewidth=3,label=r'$f_2(c)$')
ax.plot(times,CTL,color=colors[2],linestyle = ':',linewidth=3,label=r'$f_3(c)$')
...
ax.set_xlim(0,150)
plt.savefig('./Figures/Fig4_4d.png')
```
