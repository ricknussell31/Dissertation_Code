...
Bins = np.zeros((3,binsize,len(Times)))
...
for j in range(3):
    ...
    for Job in range(loops):
        ...
        for k in range(1,simTime):
            ...
            if (k in Times):
                for j in range(SM.N):
                    Dists = np.delete(Calc_Dist(POS[1,j:,j:],10),0,0)
...
## Fig. 4.9a - RDF for varied deposition functions over time.
...
for i in range(len(Times)):
    for j in range(3):
        ...
        axs[i].plot(Lengs,AN,label='$f_{0}(c)$'.format(j+1),color=colors[j],linestyle=linestyles[j],linewidth=5)
...
plt.savefig('./Figures/Fig4_9a.png')
```

## Fig. 4.9b - Average size of an aggregation over time using RDF
...
for j in range(3):
    ...
    ax.scatter(Times,FD,label='$f_{0}(c)$'.format(j+1),color=colors[j],marker=markers[j])
...
plt.savefig('./Figures/Fig4_9b.png')
```

## Fig. 4.9c - Density of Aggregation using RDF at $r= 0$, i.e. $g(r=0,t)$
...
plt.savefig('./Figures/Fig4_9c.png')
```
