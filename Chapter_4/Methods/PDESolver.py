...
class Background_Field(object):
    ...
    def BuildMatricies(self):
        ...
        self.IL = inv(self.I + (self.dt/2)*self.I)
...
class PlankChem(Background_Field):
    ...
    def firstStepSO(self,ck,pk):
        ...
        #cn = ck - .00001*cos(self.CC*self.xm)
        #pn = pk + .00001*cos(self.CC*self.xm)
        cn = ck
        pn = pk
    ...
    def SecondOrder(self,ck,pk,pk1):
        ...
        if (self.delta==0): #prevent possible division by 0
            ...
            first = np.multiply(np.divide(self.D.dot(ck),
                                          sqrt(np.multiply(self.D.dot(ck),self.D.dot(ck))+self.delta**2)),pk)

        mp = self.D.dot(first)

        fp = (self.dt**2)*(self.D2.dot(pk) - mp)

        pn = self.IL.dot(fp + 2*pk + (self.dt/2 - 1)*pk1)
