...
TKS = ['Random','Unif','Triangle','TriangleA']
...
TotalTime = 300 #float(sys.argv[1]) #Total time 
...
TotalChem[0] = np.sum(CHEM[1].flatten())*(LL/meshsize)**2
...
for k in range(1,simTime):
	...
	TotalGrad[k] = np.sum(abs(np.gradient(CHEM[1],LL/meshsize)))*(LL/meshsize)**2
	if (k in Times):
		np.save('./DOutputsTK/Pos_{3}{1}_T{0}_J{2}'.format(int(k*SM.k),int(sys.argv[3])-1,Job,TKS[Turn]),pos)
		np.save('./DOutputsTK/Chem_{3}{1}_T{0}_J{2}'.format(int(k*SM.k),int(sys.argv[3])-1,Job,TKS[Turn]),CHEM[1])
...
np.save('./DOutputsTK/PosN_{3}{1}_T{0}_J{2}'.format(TotalTime,int(sys.argv[3])-1,Job,TKS[Turn]),pos)
np.save('./DOutputsTK/ChemN_{3}{1}_T{0}_J{2}'.format(TotalTime,int(sys.argv[3])-1,Job,TKS[Turn]),CHEM[1])
np.save('./DOutputsTK/MaxMin_{3}{1}_T{0}_J{2}'.format(TotalTime,int(sys.argv[3])-1,Job,TKS[Turn]),MaxChem-MinChem)
np.save('./DOutputsTK/TotGrad_{3}{1}_T{0}_J{2}'.format(TotalTime,int(sys.argv[3])-1,Job,TKS[Turn]),TotalGrad)
