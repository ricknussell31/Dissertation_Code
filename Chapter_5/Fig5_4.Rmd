...
for m in range(len(Means)):
    ...
    for j in range(3):
        ...
        np.save('./Data/Pos_Dep{0}_UnifTK_M{1}.npy'.format(Deps[j],m),pos)
```
...
## Fig 5.4 - Plankton density at $t = 300$ for varied turning kernel means and deposition functions

```{python}
fig, axs = plt.subplots(figsize = (40,24),nrows = 3, ncols = 5, sharey = 'row', sharex = 'row')
...
for i in range(len(Means)):
    ...
    for j in range(len(Deps)):
                hts = np.save('./Data/Pos_Dep{0}_UnifTK_M{1}.npy'.format(Deps[j],Means2[i]),pos)
        xm, ym, ContoursSlice = particleProjectionFaster(hts,400)
        cs = axs[j,i].pcolormesh(xm,ym,ContoursSlice,cmap=cmaps[j])
        cs.set_clim(Low,Highs[j])
        ...
        if (i == 0):
            ...
            axs[j,0].set_ylabel(r'$f(c) = f_{0}(c)$'.format(j+1),fontsize=40,rotation=90)
        ...
        if (i == 4):
            ...
            if (j == 1):
                cbar = fig.colorbar(cs,ticks = [.5,1,1.5],
                        ax=axs[j].ravel().tolist(),location='right',pad=0.01,shrink=0.8,alpha=.3)
                cbar.set_ticks([.5,1,1.5])
                cbar.update_ticks()
                cbar.ax.tick_params(labelsize=25)
                cbar.ax.set_yticklabels([r'$\overline{\rho}/2$',r'$\overline{\rho}$',
                                         r'$3\overline{\rho}/2$'])
    ...
    axs[0,i].set_title(r'$\mu = {0}$'.format(round(Means[Means2[i]],3)),fontsize=40,pad=1)
    axs[2,i].set_xlabel(r'$x$'.format(Times[i]),fontsize=40)

plt.savefig('./Figures/Fig5_4.png')
```
